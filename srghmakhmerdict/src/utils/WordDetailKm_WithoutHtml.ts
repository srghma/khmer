import {
  strToWithoutHtml_remove_orUndefined,
  type TypedWithoutHtml,
...
type Processor<K extends keyof WordDetailKm> = (x: WordDetailKm[K]) => TypedWithoutHtml | undefined

const strToWithoutHtml_remove_orUndefined_ = undefined_lift(strToWithoutHtml_remove_orUndefined)

export const processors: { [K in keyof WordDetailKm]-?: Processor<K> } = {
  desc: x => strToWithoutHtml_remove_orUndefined_(x),
  phonetic: () => undefined,
  wiktionary: x =>
    strToWithoutHtml_remove_orUndefined_(
  ...
  from_csv_raw_html: x => strToWithoutHtml_remove_orUndefined_(x),
  from_chuon_nath: () => undefined,
  from_chuon_nath_translated: x => strToWithoutHtml_remove_orUndefined_(x),
  from_russian_wiki: x =>
    strToWithoutHtml_remove_orUndefined_(undefined_lift(wiktionary_ru__get_short_info__only_ru_text_without_html)(x)),
  gorgoniev: x => {
    if (x === undefined) return undefined
    const ipaTag = 'pre' as NonEmptyStringTrimmed

    return strToWithoutHtml_remove_orUndefined(x, [ipaTag])
  },
  en_km_com: x => strToWithoutHtml_remove_orUndefined_(x),
}

// Helper to pick the best HTML source from the DB object
export const getBestDefinitionEnOrRuFromKm = (
  detail: WordDetailKm,
): { t: ShortDefinitionKm['source']; v: TypedWithoutHtml } | undefined => {
...
export const getBestDefinitionEnOrRuFromKm_fromShort_onlyWithoutHtml = (

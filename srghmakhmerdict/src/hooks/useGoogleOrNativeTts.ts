...
export type GoogleOrNativeTtsState =
  ...
  | {
      t: 'ready'
      speak: (word: NonEmptyStringTrimmed, language: ToTranslateLanguage) => Promise<void>
    }
 ...
 */
export function useGoogleOrNativeTts(): GoogleOrNativeTtsState {
  const toast = useAppToast()

  const isSpeaking = useSyncExternalStore(speakingStore.subscribe, speakingStore.getSnapshot)

  const speak = useCallback(
    async (word: NonEmptyStringTrimmed, language: ToTranslateLanguage) => {
      if (!word) {

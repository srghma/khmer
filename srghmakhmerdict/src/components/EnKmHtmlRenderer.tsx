...
import { colorizeText, type ColorizationMode } from '../utils/text-processing'
...
export const EnKmHtmlRenderer = ({ html, km_map, colorMode }: EnKmHtmlRendererProps) => {
  ...
  const finalHtml = useMemo(() => {
    ...
    if (!ocrMap) return { __html: processHtmlImages(html, imageMode) }

    const htmlWithOcr = injectOcrIntoHtml(html, ocrMap, km_map, colorMode)
    const htmlWithOcr_ = processHtmlImages(htmlWithOcr, imageMode)

    return { __html: htmlWithOcr_ }

...
interface NonKhmerHideToggleIconProps {
  className?: string
  isEnabled: boolean // true = Words are hidden (Skeleton mode)
}

export const NonKhmerHideToggleIcon = React.memo(({ className, isEnabled }: NonKhmerHideToggleIconProps) => {
  return (
    <div className={`relative w-6 h-6 ${className}`}>
      {/* Base Layer: The NonKhmer Letter 'Ka' (ក) */}
      <div
        className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${
          isEnabled ? 'opacity-20 blur-[0.5px]' : 'opacity-100'
        }`}
      >
        <span className="font-bold text-[1.1rem] select-none leading-none pb-[2px]">A</span>
      </div>
...
NonKhmerHideToggleIcon.displayName = 'NonKhmerHideToggleIcon'

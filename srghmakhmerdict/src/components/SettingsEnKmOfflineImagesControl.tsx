...
const MdOutlineCloudDownload_ = <MdOutlineCloudDownload className="text-xl" />
...
export const SettingsEnKmOfflineImagesControl = () => {
  ...
  return (
    <div className="flex flex-col gap-3 p-3 rounded-medium bg-default-100/50 border border-default-100">
      <span className="font-semibold text-default-500 uppercase tracking-wider text-xs">{LL.OFFLINE.TITLE()}</span>

      <div className="flex justify-between items-center">
        <div className="flex flex-col">
          <span className="font-medium text-foreground text-base">{LL.OFFLINE.LABEL()}</span>
      ...
      {isDownloading && (
        <div className="flex flex-col gap-2 w-full">
          <Progress
            ...
            value={progress}
          />
          <span className="text-default-500 text-sm">{statusLabel}</span>
        </div>
      )}

      {isAvailable && !isDownloading && (
        <div className="flex items-center gap-2 font-medium px-1 text-success text-sm">
          <MdCheckCircle className="text-xl" /> {LL.OFFLINE.ASSETS_AVAILABLE({ count: hasOfflineAssets! })}
        </div>
      )}
    </div>
  )

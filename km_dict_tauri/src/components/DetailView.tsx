...
export const DetailView: React.FC<DetailViewProps> = React.memo(
  ({ word, mode, onNavigate, fontSize, onBack, km_setOfPureKhmerWords, canGoBack }) => {
    ...
    const processArray = useCallback(
      (items: NonEmptyArray<NonEmptyStringTrimmed> | undefined) => {
        ...
        return Array_toNonEmptyArray_orThrow(
          items.map(item => colorizeHtml(item, colorMode, km_setOfPureKhmerWords) ?? item),
        )
      },
      [colorMode, km_setOfPureKhmerWords],
    )

    const processedDesc = useMemo(
      () => colorizeHtml(data?.desc, colorMode, km_setOfPureKhmerWords),
      [data?.desc, colorMode, km_setOfPureKhmerWords],
    )
    const processedWiktionary = useMemo(
      () => colorizeHtml(data?.wiktionary, colorMode, km_setOfPureKhmerWords),
      [data?.wiktionary, colorMode, km_setOfPureKhmerWords],
    )
    const processedChuonNath = useMemo(
      () => colorizeHtml(data?.from_chuon_nath, colorMode, km_setOfPureKhmerWords),
      [data?.from_chuon_nath, colorMode, km_setOfPureKhmerWords],
    )
    const processedChuonNathTrans = useMemo(
      () => colorizeHtml(data?.from_chuon_nath_translated, colorMode, km_setOfPureKhmerWords),
      [data?.from_chuon_nath_translated, colorMode, km_setOfPureKhmerWords],
    )
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button
                  ...
                  isLoading={!km_setOfPureKhmerWords}
                  radius="full"
                  variant="light"
                >

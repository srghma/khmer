...
interface KhmerComplexTableModalProps {
  isOpen: boolean
  onClose: () => void
  wordList: NonEmptyStringTrimmed[]
}
...
const WordCard = memo(({ word, highlight }: { word: string; highlight: string }) => {
  const parts = useMemo(() => word.split(highlight), [word, highlight])
          ...
          <div ref={parentRef} className="w-full h-full overflow-y-auto px-4 py-4" style={{ contain: 'strict' }}>
            ...
            >
              ...
              {rowVirtualizer.getVirtualItems().map(virtualRow => {
                ...
                const word1 = data.words[firstIndex]
                const word2 = data.words[secondIndex]

                return (
                  <div
                    key={virtualRow.key}
                    className="flex gap-3 pb-3" // Padding bottom for gap
                    style={{
                      position: 'absolute',
                      top: 0,
                      left: 0,
                  ...
                  >
                    <div className="flex-1 min-w-0">{word1 && <WordCard highlight={data.title} word={word1} />}</div>
                    <div className="flex-1 min-w-0">{word2 && <WordCard highlight={data.title} word={word2} />}</div>
                  </div>
                )
              })}
            </div>
...
const MatrixCell = memo(
  ({
    ...
    const vowelSuffix = cleanVowel(vowelRaw)
        ...
        <table className="w-full table-fixed border-collapse">
          <tbody>
            {vowelsGrid.map((row, rIdx) => (
              <tr key={rIdx}>
                {row.map(v => {
                  const vowelSuffix = cleanVowel(v)
                  const combo = consonant + vowelSuffix
                  const count = index?.get(combo)?.length ?? 0
                  const cellId = `c-${combo}`

                  return <MatrixCell key={cellId} consonant={consonant} count={count} vowelRaw={v} onClick={onClick} />
                })}
              </tr>
            ))}
          </tbody>
...
export const KhmerComplexTableModal: React.FC<KhmerComplexTableModalProps> = ({ isOpen, onClose, wordList }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      console.log(`[KhmerComplexTable] Indexed ${wordList.length} words in ${(performance.now() - t0).toFixed(2)}ms`)
      setIndex(newIndex)
      setIsProcessing(false)
    }, 100)

    return () => clearTimeout(timer)
  }, [isOpen, wordList, index])
  ...
  return (
    <>
            ...
            <div className="flex gap-4 items-center">
              ...
              {!isProcessing && index && (
                <div className="text-sm font-normal text-default-500 flex gap-4 border-l border-divider pl-4">
                  ...
                  <span className="text-default-400">({wordList.length} words indexed)</span>
                </div>
              )}
            </div>
          </ModalHeader>
                      ...
                      {independentVowels.map(char => {
                        ...
                        return (
                          <button
                            key={char}
                            className={`
                              border border-default-300 bg-content1 rounded-lg p-4 flex flex-col items-center gap-1 transition-all shadow-sm
                              ${count === 0 ? 'opacity-50 cursor-default' : 'cursor-pointer hover:border-primary hover:bg-primary/5 hover:scale-105'}
                            `}
                            disabled={count === 0}
      ...
      {/* Nested Modal for the selected Deck */}

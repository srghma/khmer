...
export const colorizeHtml = (
  ...
  if (mode === 'dictionary' && !km_setOfPureKhmerWords) return html as NonEmptyStringTrimmed
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (textContent && /[\p{Script=Khmer}]/u.test(textContent)) {
        ...
        const newHtml = textContent.replace(/[\p{Script=Khmer}]+/gu, match_ => {
          ...
          } else if (mode === 'dictionary' && km_setOfPureKhmerWords) {
            words = khmerSentenceToWords_usingDictionary(match, km_setOfPureKhmerWords)
          }
          ...
          return words
            .map(w => {
              ...
              if (mode === 'dictionary' && km_setOfPureKhmerWords) {
                if (km_setOfPureKhmerWords.has(w)) {
                  const color = COLOR_PALETTE[wordCounter % COLOR_PALETTE.length]

...
// Using the names found in the existing project structure
...
// The actual struct sent to frontend with parsed arrays
...
// Add a specific command to check readiness
#[command]
...
pub async fn get_en_words(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let pool = state.get_pool().await?; // Use the helper from lib.rs

    let sql = format!(
        "SELECT Word FROM {} WHERE Desc IS NOT NULL ORDER BY Word ASC",
        TBL_EN
    );
...
#[command]
pub async fn get_km_words(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let pool = state.get_pool().await?;

    let sql = format!("SELECT Word FROM {} ORDER BY Word ASC", TBL_KM);
    let rows = sqlx::query_as::<_, WordRow>(&sql)
        .fetch_all(&pool)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn get_word_detail_en(
...
) -> Result<Option<WordDetailGeneric>, String> {
    let pool = state.get_pool().await?;
    ...
    let row = sqlx::query_as::<_, WordDetailGeneric>(&sql)
        .bind(word)
        .fetch_optional(&pool)
        .await
        .map_err(|e| e.to_string())?;
...
// --- Content Search Commands (Apply same pattern) ---
#[command]
pub async fn search_en_content(
    ...
    // Limit to 50 results to prevent UI flooding; prefer shorter words as they might be more relevant or easier to read
    let sql = format!(

...
import {
  ...
  element_to_HTMLIFrameElement_orThrow,
} from "../utils/dom"
...
export const visibleDriveIframe = (): HTMLIFrameElement | undefined => {
  const iframes: HTMLIFrameElement[] = Array.from(
    document.querySelectorAll('iframe[src*="docs.google.com/picker"]'),
  )
  ...
  if (iframes.length !== 0 && iframes.length !== 1) {
    ...
    throw new Error(
      `visibleDriveIframe: can be only 0 or 1, but got ${iframes.length}`,
    )
  }
...
export const getAiStudioState = () => {
  const uploadedFiles = Array.from(
    document.querySelectorAll("mat-chip-row, .file-chip"),
  ).map((el) => el.textContent?.trim() || "file")
  ...
  return {
    ...
    isGenerating:
      document
        .querySelector("ms-run-button button")
...
export const openDriveIframe = async () => {
  ...
  if (!driveItem) {
    ...
    driveItem =
      menuItems.find((el) => el.textContent?.toLowerCase().includes("drive")) ||
      null
  }

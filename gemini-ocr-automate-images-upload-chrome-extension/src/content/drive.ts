...
import {
  assert_element_is_HTMLElement,
  element_to_HTMLElement_orThrow,
} from "../utils/dom"
...
import { scrollAndProcess, ScrollAndProcessResponse } from "./scroll"
...
const getIsRowSelected = (row: HTMLElement): boolean =>
  ...
  row.classList.contains("picker-selected")
...
const extractPageNumber = (
  ...
  return match && match[1]
    ? strOrNumberToNonNegativeIntOrThrow_strict(match[1])
    : undefined
}
...
async function ensureListView() {
  const listBtn = document.querySelector(
    'button[aria-label="List View"], button[data-id="list"]',
  )
  const isVisible =
    listBtn &&
    element_to_HTMLElement_orThrow(listBtn).offsetParent !== undefined
...
const getRows = async (container: HTMLElement): Promise<RowData[]> => {
  const rows: RowData[] = Array.from(
    container.querySelectorAll('div[role="option"]'),
  )
...
export async function selectManyFiles(
  ...
  const scrollResult: ScrollAndProcessResponse<R, R> = await scrollAndProcess({
    ...
    tryConsume: async () => {
      ...
      if (selected_and_should_not_be_selected.length > 0)
        throw new Error(
          "selected_and_should_not_be_selected: Who the fuck clicked?",
        )

      const already_selected_or_will_be =
        selected_and_should_be_selected.concat(
          not_selected_and_should_be_selected,
        )
  ...
  if (scrollResult.t === "error_max_attempts_exceeded")
    throw new Error("error_max_attempts_exceeded")
  ...
  for (const {
    ...
    pageNumber,
    isSelected,
  } of result.not_selected_and_should_be_selected) {
    if (isSelected)
      throw new Error(
        "Impossible: not_selected_and_should_be_selected isSelected",
      )
    ...
    if (!getIsRowSelected(row))
      throw new Error(
        "Impossible: not_selected_and_should_be_selected -> clicked but didnt select",
      )
  }

  const selected = [
    ...result.not_selected_and_should_be_selected,
    ...result.selected_and_should_be_selected,
  ]

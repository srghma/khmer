...
export const doScroll = (container: HTMLElement, scrollStepDelay: number) => {
  ...
  return async (): Promise<boolean> => {
    ...
    const scrollingDownWasSuccessful =
...
export async function scrollAndProcess<Result, PartialResult>({
  ...
  scrollStepDelay,
  tryConsume,
}: ScrollOptions<Result, PartialResult>): Promise<
  ScrollAndProcessResponse<Result, PartialResult>
> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (consumeResult.t === "i_consumed_all_im_full")
      return { t: "success", result: consumeResult.result }

    const scrollingDownWasSuccessful = await confirmStable(() => scroll(), {
      nOfRetries: 3,
      doesValueRequireConfirmation: (scrollingDownWasSuccessful: boolean) =>
        scrollingDownWasSuccessful === false,
    })

...
const fetchWiktionaryHtml = async (word: NonEmptyStringTrimmed): Promise<Except<string, FetchResult>> => {
  try {
...
export const processQueue = async (words: Set<NonEmptyStringTrimmed>, cache: WiktionaryCache): Promise<void> => {
  ...
  const addWordToQueue = (word: NonEmptyStringTrimmed, attempt = 1) => {
    queue.add(
      async () => {
        ...
        console.log(`[Fetch] [${processedCount} ops] ${word} (Attempt ${attempt}, Concurrency: ${queue.concurrency})`)

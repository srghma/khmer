...
export function zipStrictObjects<A, B>(
  a: Record<string, A>,
  b: Record<string, B>,
): Record<string, [A, B]> {
  ...
  if (missingInA.length > 0 || missingInB.length > 0) {
    throw new Error(
      `zipStrictObjects: keys do not match.\n` +
        (missingInA.length > 0
          ? `Extra keys in B: ${missingInA.join(", ")}\n`
          : "") +
        (missingInB.length > 0
          ? `Extra keys in A: ${missingInB.join(", ")}\n`
          : ""),
    )
  }
...
export function zipStrictObjectsValues<A, B>(
  a: Record<string, A>,
  b: Record<string, B>,
): [A, B][] {
...
export function zipStrictObjects3<A, B, C>(
  ...
  if (
    missingInB.length > 0 ||
    missingInC.length > 0 ||
    missingInAfromB.length > 0 ||
    missingInAfromC.length > 0
  ) {
    throw new Error(
      `zipStrictObjects3: keys do not match.\n` +
        (missingInB.length > 0
          ? `Extra keys in A not in B: ${missingInB.join(", ")}\n`
          : "") +
        (missingInC.length > 0
          ? `Extra keys in A not in C: ${missingInC.join(", ")}\n`
          : "") +
        (missingInAfromB.length > 0
          ? `Extra keys in B not in A: ${missingInAfromB.join(", ")}\n`
          : "") +
        (missingInAfromC.length > 0
          ? `Extra keys in C not in A: ${missingInAfromC.join(", ")}\n`
          : ""),
    )
  }

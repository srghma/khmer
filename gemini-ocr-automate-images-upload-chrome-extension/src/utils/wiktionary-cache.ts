...
function isCacheEntry_fromUnknown(v: unknown): v is CacheEntry {
  ...
  // Must have a word
  if (!("word" in candidate) || !unknown_isNonEmptyString(candidate.word))
    return false
  ...
  if ("html" in candidate && unknown_isNonEmptyString(candidate.html))
    return true
  return false
}

function assertIsCacheEntry_fromUnknown(v: unknown): asserts v is CacheEntry {
  if (!isCacheEntry_fromUnknown(v))
    throw new TypeError(
      `Invalid CacheEntry shape. Received: ${JSON.stringify(v)}`,
    )
}
...
const ensureCacheFileExists = async (
  CACHE_FILE: NonEmptyString,
): Promise<void> => {
  try {
...
const readCacheEntries = (
  CACHE_FILE: NonEmptyString,
): Promise<Map<NonEmptyString, CacheEntry>> => {
...
export const WiktionaryCache_create = async (
  CACHE_FILE: NonEmptyString,
): Promise<WiktionaryCache> => {
  ...
  return {
    ...
    get: (word) => {
      ...
      if (!("html" in entry))
        throw new Error(`Cache entry for ${word} is missing html field`)

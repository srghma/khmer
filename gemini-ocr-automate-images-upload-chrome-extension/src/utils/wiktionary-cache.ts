...
import { unknown_isNonEmptyStringTrimmed, type NonEmptyStringTrimmed } from './non-empty-string-trimmed'
...
function isCacheEntry_fromUnknown(v: unknown): v is CacheEntry {
  ...
  // Must have a word
  if (!('word' in candidate) || !unknown_isNonEmptyStringTrimmed(candidate.word)) return false
  ...
  if ('html' in candidate && unknown_isNonEmptyStringTrimmed(candidate.html)) return true
  return false
}
...
const ensureCacheFileExists = async (CACHE_FILE: NonEmptyStringTrimmed): Promise<void> => {
  try {
...
const readCacheEntries = (CACHE_FILE: NonEmptyStringTrimmed): Promise<Map<NonEmptyStringTrimmed, CacheEntry>> => {
  return new Promise((resolve, reject) => {
    const map = new Map<NonEmptyStringTrimmed, CacheEntry>()
...
const appendToCacheFile = async (CACHE_FILE: NonEmptyStringTrimmed, entry: CacheEntry): Promise<void> => {
...
export const WiktionaryCache_create = async (CACHE_FILE: NonEmptyStringTrimmed): Promise<WiktionaryCache> => {

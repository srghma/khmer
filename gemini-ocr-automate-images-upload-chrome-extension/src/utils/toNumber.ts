...
// global config
export const validNumber__maxDigitsBeforeDot: ValidNonNegativeInt =
  20 as ValidNonNegativeInt
export const validNumber__maxDigitsAfterDot: ValidNonNegativeInt =
  10 as ValidNonNegativeInt // I want to allow seeing 1.1 and 1.12 and 1.1234567890, but not 1.12345678901
...
for (const [input, expected_TT, expected_TF, expected_FT, expected_FF] of [
  ...
  const result_TF = numberToValidNonNegativeNumberOrUndefined(
    input,
    true,
    false,
  )
  const result_FT = numberToValidNonNegativeNumberOrUndefined(
    input,
    false,
    true,
  )
  const result_FF = numberToValidNonNegativeNumberOrUndefined(
    input,
    false,
    false,
  )
...

{
  ...
  for (const [input, expected] of testCases) {
    permutations.forEach(([round, clamp], i) => {
      ...
      if (result !== exp) {
        console.error(
          `❌ numberToValidPercentOrUndefined${ctx} -> Expected: ${exp} | Got: ${result}`,
        )
      }
    })
  }
...
for (const [
  input,
  expected_strToNumberOrUndefined_lenient,
  expected_strToNumberOrUndefined_strict,
] of [
  // clean numbers
  ["42", 42, 42],
  ["42.5", 42.5, 42.5],
...

{
  ...
  const testCases: [
    ...
    [
      "10.11159",
      [10.11159, 10.11159, 10.11159, 10.11159],
      [10.11159, 10.11159, 10.11159, 10.11159],
    ],
    [
      "99.123456789012",
  ...
  for (const [input, expectedLenient, expectedStrict] of testCases) {
    permutations.forEach(([round, clamp], i) => {
      ...
      if (resultL !== expectedL) {
        console.error(
          `❌ strToPercentOrUndefined_lenient${ctx} -> Expected: ${expectedL} | Got: ${resultL}`,
        )
      }

      if (resultS !== expectedS) {
        console.error(
          `❌ strToPercentOrUndefined_strict${ctx} -> Expected: ${expectedS} | Got: ${resultS}`,
        )
      }
    })
  }
...
for (const [
  ...
  if (result_lenient !== expected_lenient)
    console.error(
      `strToIntOrUndefined_lenient: for "${input}" result=${result_lenient} expected=${expected_lenient}`,
    )
...
for (const [
  ...
  const result_strict_nonrounded = strToNonNegativeIntOrUndefined_strict(
    input,
    false,
  )
  const result_strict_rounded = strToNonNegativeIntOrUndefined_strict(
    input,
    true,
  )
  if (result_lenient !== expected_lenient)

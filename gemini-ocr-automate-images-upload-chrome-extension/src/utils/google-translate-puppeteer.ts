...
export async function translateBulk({
  ...
  { original: NonEmptyStringTrimmed; translation: NonEmptyStringTrimmed }[]
> {
  console.log(
    "translateBulk: listOfWordsToTranslate.length",
    listOfWordsToTranslate.length,
  )
    ...
    const results: {
      ...
      async (word: NonEmptyStringTrimmed, signal?: AbortSignal) => {
        ...
        try {
          ...
          console.warn(
            `[translateBulk] Error translating "${word}":`,

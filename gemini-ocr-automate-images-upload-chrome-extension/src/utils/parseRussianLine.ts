...
const partOfSpeech: Parser<TokenKind, PartOfSpeech> = apply(
  italicContent,
  (content) => {
...
const grammarInfo: Parser<TokenKind, GrammarInfo> = apply(
  italicContent,
  (content) => ({
    type: "GrammarInfo",
    content,
...
const italicToken: Parser<
  TokenKind,
  UslSocr | Sex | PartOfSpeech | GrammarInfo
> = alt(sexMarker, uslSocr, partOfSpeech, grammarInfo)
...
// Roman numeral
const romanNumber: Parser<TokenKind, RomanNumber> = apply(
  tok(TokenKind.RomanNumeral),
  (token) => ({
    type: "RomanNumber",
    value: token.text,
  }),
...
// Plain Russian word
const plainRussianWord: Parser<
  TokenKind,
  { type: "RussianWord"; word: string }
> = apply(tok(TokenKind.RussianWord), (token) => ({
  type: "RussianWord",
  word: token.text,
}))

// Plain Khmer word
const plainKhmerWord: Parser<TokenKind, { type: "KhmerWord"; word: string }> =
  apply(tok(TokenKind.KhmerWord), (token) => ({
    type: "KhmerWord",
    word: token.text,
  }))

// Simple tokens
const comma: Parser<TokenKind, { type: "Comma" }> = apply(
  tok(TokenKind.Comma),
  () => ({ type: "Comma" }),
)

const semicolon: Parser<TokenKind, { type: "Semicolon" }> = apply(
  tok(TokenKind.Semicolon),
  () => ({ type: "Semicolon" }),
)

const diamond: Parser<TokenKind, { type: "Diamond" }> = apply(
  tok(TokenKind.Diamond),
  () => ({ type: "Diamond" }),
)

const space: Parser<TokenKind, { type: "Space" }> = apply(
  tok(TokenKind.Space),
  () => ({ type: "Space" }),
)

...
} from '@gemini-ocr-automate-images-upload-chrome-extension/utils/wiktionary-cache-db.js' // Changed to DB version
...
import { DictionaryManager } from '@gemini-ocr-automate-images-upload-chrome-extension/utils/dictionary'
...
const migrateJsonlToDb = async (cache: WiktionaryCache) => {
  ...
  for await (const row of parser) {
    ...
    if (entry.word && unknown_isNonEmptyStringTrimmed(entry.word)) {
      ...
      // Check if already in DB to avoid unnecessary writes, though upsert handles it
      if (!cache.has(word)) {
        if (entry.status === 404) {
...
const main = async () => {
  ...
  // Convert dictionary words to NonEmptyStringTrimmed for the fetcher
  // (Assuming TypedKhmerWord is compatible with string, checking non-empty)
  ...
  if (inputFile && outputFile) {
    ...
    console.log(`Added words from input file. Total unique words to process: ${wordsToFetch.size}`)
  } else {
    console.log(`No input/output files provided. Processing ${wordsToFetch.size} words from dictionary only.`)
  }

  // 5. Run Fetch Pipeline
  // This will fill the SQLite DB with missing entries
  await processQueue(wordsToFetch, cache)

  // 6. Generate Output CSV (Only if input/output files were provided)
  if (inputFile && outputFile) {
    ...
    const outputRows: string[] = cards
      .map(card => {
        ...
        const cleanedHtmls: (NonEmptyStringTrimmed | undefined)[] = card.words.map(word => {
          const entry = cache.get(word) // No assertIsDefinedAndReturn here, simpler logic
